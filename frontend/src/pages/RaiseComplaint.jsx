...
export default function RaiseComplaint({ onSendToRepair }) {
  ...
  const selectSymptomTag = (tag) => {
    if (!complaint.includes(tag)) {
      setComplaint(prev =>
        prev
          ? `${prev} Experiencing ${tag.toLowerCase()}.`
          : `Experiencing ${tag.toLowerCase()}.`
      );
    }
  };
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">
          Raise PC Complaint & AI Diagnostics
        </h1>

        <p className="text-gray-400 text-sm">
          Analyze sensor telemetry and query historical completed repairs dynamically.
        </p>
      </div>
              ...
              {filteredPcs.map(pc => (
                ...
                >
                  ...
                  <span className="text-gray-500">
                    {pc.model_name.split(' ').slice(0, 2).join(' ')}
                  </span>
                </button>
              ))}
            ...
            {selectedPc && (
              <div className="p-3.5 bg-[#111827] border border-[#1F2937] rounded space-y-1 text-xs">
                ...
                <p className="text-gray-500 text-[10px] mt-2">
                  Last Updated:{' '}
                  {new Date(selectedPc.last_updated).toLocaleString()}
                </p>
              </div>
            )}
          </div>
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          {selectedPc && (
            <div className="bg-[#0B0F19] border border-[#1F2937] p-5 rounded-lg space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                ...
                {/* Temperature */}
                <div
                  className={`p-3 rounded border bg-[#030712] ${
                    selectedPc.temperature > 75.0
                ...
                >
                  ...
                  <p className="text-lg font-bold mt-1 text-white">
                    {selectedPc.temperature.toFixed(1)}°C
                  </p>
                  ...
                  <div className="w-full bg-[#111827] h-1 rounded mt-2 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(
                          100,
                          (selectedPc.temperature / 110) * 100
                        )}%`
                      }}
                    />
                  </div>
                ...
                >
                  ...
                  <p className="text-lg font-bold mt-1 text-white">
                    {selectedPc.fan_speed.toFixed(0)} RPM
                  </p>
                ...
                >
                  ...
                  <p className="text-lg font-bold mt-1 text-white">
                    {selectedPc.cpu_usage.toFixed(1)}%
                  </p>
                ...
                >
                  ...
                  <p className="text-lg font-bold mt-1 text-white">
                    {selectedPc.ram_usage.toFixed(1)}%
                  </p>
                ...
                <div
                  className={`p-3 rounded border bg-[#030712] ${
                    Math.abs(selectedPc.voltage - 15.0) > 3.0
                      ? 'border-red-500/30 text-red-500'
                      : 'border-[#1F2937] text-gray-300'
                  }`}
                >
                  ...
                  <p className="text-lg font-bold mt-1 text-white">
                    {selectedPc.voltage.toFixed(2)}V
                  </p>
                ...
                >
                  ...
                  <p className="text-lg font-bold mt-1 text-white">
                    {selectedPc.disk_usage.toFixed(1)}%
                  </p>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-[#0B0F19] border border-[#1F2937] p-5 rounded-lg space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="p-3 bg-[#030712] border border-[#1F2937] rounded">
                    ...
                    <p className="text-[10px] text-gray-400 mt-0.5">
                      ...
                      <b className="text-cyan-400">
                        {analysisResult.problem_analysis.agreement_status}
                      </b>
                    </p>
                  </div>
                  ...
                  <div className="p-3 bg-[#030712] border border-[#1F2937] rounded">
                    <p className="text-[10px] text-gray-500 uppercase font-medium">
                      Health Score (Surrogate)
                    </p>

                    <div className="flex items-baseline space-x-2 mt-1">
                      <span className="text-xl font-bold text-white">
                        {analysisResult.predictive_health.health_score}
                      </span>

                      <span
                        className={`px-1.5 py-0.5 rounded text-[9px] font-semibold ${getHealthColor(
                  ...
                  <div className="p-3 bg-[#030712] border border-[#1F2937] rounded">
                    ...
                    <h4
                      className={`text-base font-bold mt-1 ${
                        analysisResult.anomaly.label === 'Abnormal'
                          ? 'text-red-400'
                          : 'text-emerald-400'
                      }`}
                    >
                    ...
                    <p className="text-[10px] text-gray-500">
                      Score: {analysisResult.anomaly.score.toFixed(3)}
                    </p>
                  </div>

                  <div
                    className={`p-3 bg-[#030712] border rounded border-l-4 ${getRiskColor(
                      analysisResult.predictive_health.risk_level
                    )}`}
                  >
                    <p className="text-[10px] text-gray-500 uppercase font-medium">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="text-right">
                      <span
                        className={`px-2 py-1 rounded text-[10px] font-bold ${
                          analysisResult.predictive_health.will_fail_soon
                  ...
                  <div className="p-3 bg-[#030712] border border-[#1F2937] rounded">
                    ...
                    <p className="text-[9px] text-gray-500 mt-0.5">
                      Surrogate model approximation based on degradation curves.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#0B0F19] border border-[#1F2937] p-5 rounded-lg space-y-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-[9px] bg-[#111827] text-gray-400 px-1.5 py-0.5 rounded border border-[#1F2937]">
                    Local Feature Attributions
                  </span>
                </div>

                <div className="space-y-2.5">
                  {analysisResult.explainability.top_contributing_features.map(
                    (feat, idx) => (
                      <div
                        key={idx}
              ...
              <div className="bg-[#0B0F19] border border-[#1F2937] p-5 rounded-lg space-y-4">
                <h3 className="text-sm font-semibold text-white">
                  Top 3 Similar Historical Cases (Semantic retrieval index)
                </h3>

                <div className="space-y-3">
                  {analysisResult.similar_cases.map(caseItem => (
                    <div
                      key={caseItem.repair_id}
                      className="bg-[#030712] border border-[#1F2937] rounded overflow-hidden"
                    >
                      <button
                        onClick={() =>
                          setExpandedRepairId(
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          <span className="text-xs font-bold text-cyan-400">
                            {caseItem.rank}. {caseItem.repair_id}
                          </span>
                      ...
                      {expandedRepairId === caseItem.repair_id && (
                        <div className="px-4 pb-4 border-t border-[#1F2937] pt-3 text-xs space-y-3">
                          ...
                          <div>
                            ...
                            <p className="text-white">
                              {caseItem.technician_notes || 'None'}
                            </p>
                          </div>
                  ...
                  {analysisResult.similar_cases.length === 0 && (
                    <p className="p-4 text-center text-gray-500 text-xs">
                      No historical matching cases found in index.
                    </p>
                  )}
                </div>
              </div>
                ...
                <div className="space-y-4 text-xs">
                  <div>
                    ...
                    <p className="text-white font-medium mt-0.5">
                      {analysisResult.recommendation.primary_recommendation}
                    </p>
                  </div>

                  <div>
                    <p className="text-gray-400 font-semibold mb-1.5">
                      Recommended Diagnostic Sequence:
                    </p>

                    <ol className="list-decimal list-inside space-y-1 text-gray-300">
                      {analysisResult.recommendation.diagnostic_sequence.map(
                        (step, idx) => (
                          <li key={idx}>
                            {step}
                          </li>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <ul className="list-disc list-inside space-y-1 text-gray-300 mt-1">
                        {analysisResult.recommendation.immediate_actions.map(
                          (act, idx) => (
                            <li key={idx}>
                              {act}
                            </li>
                    ...
                    <div>
                      ...
                      <ul className="list-disc list-inside space-y-1 text-gray-300 mt-1">
                        {analysisResult.recommendation.preventive_actions.map(
                          (act, idx) => (
                            <li key={idx}>
                              {act}
                            </li>
                  ...
                  <div className="p-3 bg-[#030712] border border-[#1F2937] rounded-lg">
                    ...
                    <p className="text-[10px] text-gray-400 mt-0.5 leading-normal">
                      AI-generated decision support based on current telemetry
                      and historical repair evidence. Final diagnosis should be
                      confirmed by qualified technical personnel.
                    </p>
                  </div>
                </div>
                  ...
                  <button
                    onClick={() =>
                      onSendToRepair({
                        ...
                        symptoms:
                          analysisResult.similar_cases[0]?.symptoms ||
                          complaint,
                        problem_detected:
                          analysisResult.problem_analysis.final_assessment,
          ...
          {!selectedPc && (
            <div className="h-64 border border-dashed border-[#1F2937] rounded-lg flex flex-col justify-center items-center text-center p-6 text-gray-500">
              ...
              <p className="text-sm">
                Please select a PC from the left panel to begin diagnostic analysis.
              </p>
            </div>
          )}
        </div>
